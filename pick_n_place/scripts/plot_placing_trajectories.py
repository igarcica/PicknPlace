...
place_vertical = np.array([
    [0.28, 0.4, 0], #Check def
    [0.12, 0.400068, 0],
    [0.120931, 0.055, 0],
    [0.0509354, 0.0559357, 0],
    [0.0517381, 0.35, 0] #High pos
])
## Diagonal placing
place_diagonal = np.array([
    [0.28, 0.4, 0], #Check def
    [0.37, 0.25, 5*np.pi/3], #pre place diagonal
    [0.185, 0.185, 11*np.pi/6], #place diagonal1
    [0.12, 0.06, 0], #place diagonal2
    [0.0500253, 0.0608235, 0], # post place
    [0.0510004, 0.35, 0] #High pos
])
## Rotating placing
place_rot = np.array([
    [0.28, 0.4, 0], #Check def -0.291506,
    [0.45, 0.25, 3*np.pi/2], #pre place rotating -0.28,
    [0.22, 0.155, 11*np.pi/6], # place rotating -0.279989,
    [0.12, 0.06, 0], #place222 -0.279994,
    [0.0500055, 0.059968, 0], #post place -0.279979,
    [0.0507902, 0.35, 0] #High pos -0.27985,
])
# ## Diagonal piling
# # waypoints = np.array([
# #     [0.28, 0.4], #Check def
...
# Plot waypoints as white dots
ax.scatter(x, y, c='white', edgecolors='black', s=100, label="Waypoints", zorder=3)

# Plot **straight-line** connections between waypoints
ax.plot(x, y, linestyle='-', linewidth=2, color='cyan', label="Vertical")
...
waypoints = place_diagonal
x = waypoints[:, 0]
y = waypoints[:, 1]
ax.plot(x, y, linestyle='-', linewidth=2, color='green', label="Diagonal")
# Plot orientation (theta) as arrows
for px, py, theta in waypoints:
    ax.arrow(px, py, 0.05 * np.cos(theta), 0.05 * np.sin(theta),
             head_width=0.01, head_length=0.01, fc='yellow', ec='yellow')
...
x = waypoints[:, 0]
y = waypoints[:, 1]
ax.plot(x, y, linestyle='-', linewidth=2, color='red', label="Rotating")
# Plot orientation (theta) as arrows
for px, py, theta in waypoints:
    ax.arrow(px, py, 0.05 * np.cos(theta), 0.05 * np.sin(theta),
             head_width=0.01, head_length=0.01, fc='yellow', ec='yellow')
