...
namespace KCL_rosplan {
	...
	bool RPTutorialInterface::concreteCallback(const rosplan_dispatch_msgs::ActionDispatch::ConstPtr& msg) {
		...
		ROS_WARN("KCL: Sending PDDL action (%s)", msg->name.c_str());
		...
		ROS_INFO("KCL: Waiting for action server to start.");
  		ac.waitForServer(); //wait for the action server to start. will wait for infinite time
  		ROS_INFO("KCL: Action server started, sending goal.");
		...
		if (finished_before_timeout)
  		{
    		actionlib::SimpleClientGoalState state = ac.getState();
  	  		ROS_INFO("KCL: Action finished: %s",state.toString().c_str());
			...
			if(int(result->done_action))
				ROS_INFO("KCL: Action ended successfully.");
			else
				ROS_INFO("KCL: Action failed.");
		...
		{
   			ROS_INFO("KCL: Action did not finish before the time out.");
			ac.cancelGoal();
			return false;
		}
